...
class HashingSink : ForwardingSink {
    ...
    companion object {
        ...
        @JvmStatic
        fun md5(sink: Sink) = HashingSink(sink, "MD5")

        /** Returns a sink that uses the obsolete SHA-1 hash algorithm to produce 160-bit hashes. */
        @JvmStatic
        fun sha1(sink: Sink) = HashingSink(sink, "SHA-1")

        /** Returns a sink that uses the SHA-256 hash algorithm to produce 256-bit hashes. */
        @JvmStatic
        fun sha256(sink: Sink) = HashingSink(sink, "SHA-256")

        /** Returns a sink that uses the SHA-512 hash algorithm to produce 512-bit hashes. */
        @JvmStatic
        fun sha512(sink: Sink) = HashingSink(sink, "SHA-512")

        /** Returns a sink that uses the obsolete SHA-1 HMAC algorithm to produce 160-bit hashes. */
        @JvmStatic
        fun hmacSha1(sink: Sink, key: ByteString) = HashingSink(sink, key, "HmacSHA1")

        /** Returns a sink that uses the SHA-256 HMAC algorithm to produce 256-bit hashes. */
        @JvmStatic
        fun hmacSha256(sink: Sink, key: ByteString) = HashingSink(sink, key, "HmacSHA256")

        /** Returns a sink that uses the SHA-512 HMAC algorithm to produce 512-bit hashes. */
        @JvmStatic
        fun hmacSha512(sink: Sink, key: ByteString) = HashingSink(sink, key, "HmacSHA512")
    }
}
