...
class HashingSource : ForwardingSource {
    ...
    companion object {
        ...
        @JvmStatic
        fun md5(source: Source) = HashingSource(source, "MD5")

        /** Returns a sink that uses the obsolete SHA-1 hash algorithm to produce 160-bit hashes. */
        @JvmStatic
        fun sha1(source: Source) = HashingSource(source, "SHA-1")

        /** Returns a sink that uses the SHA-256 hash algorithm to produce 256-bit hashes. */
        @JvmStatic
        fun sha256(source: Source) = HashingSource(source, "SHA-256")

        /** Returns a sink that uses the SHA-512 hash algorithm to produce 512-bit hashes. */
        @JvmStatic
        fun sha512(source: Source) = HashingSource(source, "SHA-512")

        /** Returns a sink that uses the obsolete SHA-1 HMAC algorithm to produce 160-bit hashes. */
        @JvmStatic
        fun hmacSha1(source: Source, key: ByteString) = HashingSource(source, key,
                "HmacSHA1")
